...
// 	err = tx.QueryRow(`select tid,type,user_id,in_balance,status from gex_order where order_id=$1 for update`, orderID).Scan(&order.TID, &order.Type, &order.UserID, &order.InBalance, &order.Status)
// 	if err != nil {
...
// 		return
// 	}
// 	err = tx.ExecRow(`update gex_order set filled=$1,in_filled=$1,status=$2,notify_result=$3 where tid=$4`, order.InFilled, OrderStatusDone, converter.JSON(result), order.TID)
// 	if err != nil {
