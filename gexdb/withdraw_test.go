...
func TestGoldbar(t *testing.T) {
	...
	goldbar, err := CreateGoldbar(ctx, user.TID, 1, xtime.Now(), "name", "phone", "address")
	...
	goldbar00, err := CreateGoldbar(ctx, user.TID, 1, xtime.Now(), "name", "phone", "address")
	...
	pgx.MockerSetCall("Pool.Begin", 1, "Rows.Scan", 1, 2, 3).ShouldError(t).Call(func(trigger int) (res xmap.M, err error) {
		_, err = CreateGoldbar(ctx, user.TID, 1, xtime.Now(), "name", "phone", "address")
		return
	})
	pgx.MockerSetCall("Pool.Begin", 1, "Rows.Scan", 1, 2).ShouldError(t).Call(func(trigger int) (res xmap.M, err error) {
		_, err = CancelGoldbar(ctx, user.TID, goldbar.OrderID, "test")
